pipeline {
    ...
    stages {
        ...
        stage('Test yamllint and ansible-lint') {
            steps {
                sh "yamllint ${env.myPlaybookName}.yml"
                sh "ansible-lint ${env.myPlaybookName}.yml"
            }
        }
        stage('Execute Ansible') {
            steps {
                 withCredentials([string(credentialsId: 'BECOME_PASS', variable: 'BECOME_PASS')]) {
                    ansiblePlaybook vaultCredentialsId: 'BECOME_PASS', credentialsId: 'ertanAnsId', disableHostKeyChecking: true, installation: 'ansible', inventory: 'dev.inv', playbook: "${env.myPlaybookName}.yml", extras: '-e ansible_become_password=${BECOME_PASS}'
                }
            }
        }
